        ...
        if st.button("Process Invoice", type="primary"):
            with st.spinner("Uploading to Backend..."):
                try:
                    ...
                    # 2. Trigger Process
                    resp = requests.post(f"{API_URL}/process", json={"run_id": run_id})
                    resp.raise_for_status()
                    ...
                    while not completed:
                        status_resp = requests.get(f"{API_URL}/status/{run_id}")
                        status_resp.raise_for_status()
                        current_state = status_resp.json()
                        
                        outcome = current_state.get("payment_status") or current_state.get("approval_status")
                        logs = current_state.get("logs", [])
                        
                        # Update status
                        last_log = logs[-1]["agent"] if logs else "Initializing..."
                    ...
                    # --- Result Display ---
                    outcome = final_state.get("payment_status") or final_state.get("approval_status")
                    ...
                    st.error("❌ Could not connect to Backend API. Is `server.py` running?")
            ...
            for line in reversed(lines[-5:]):
                try:
                    ...
                    with st.container(border=True):
                        st.markdown(f":{color}[**{outcome.upper()}**] - {log_entry.get('timestamp')}")
