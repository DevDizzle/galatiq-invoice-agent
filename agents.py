...
def ingestion_agent(state: GlobalState) -> GlobalState:
    ...
    if state.retry_count > 3:
        ...
        utils.log_action(state, "IngestionAgent", "Retry limit hit", [], error_msg)
        return state
    ...
    try:
        ...
        utils.log_action(state, "IngestionAgent", f"Failed processing {state.invoice_file_path}", [], str(e))
    return state
...
def validation_agent(state: GlobalState) -> GlobalState:
    ...
    errors = []
    tool_calls = []
    # If ingestion failed, skip validation logic but return state so router can send back to ingestion
...
def payment_agent(state: GlobalState) -> GlobalState:
    ...
    if state.approval_status == "APPROVED":
        result = mock_payment(state.extracted_data.vendor, state.extracted_data.amount)
